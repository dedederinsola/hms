        ...
        echo '<table class="table table-bordered noprint" border="2" cell cellspacing="15" cellpadding="5" width="1000"> 
            ...
            if (empty($tableNames)) {
                ...
                foreach ($tableNames as $tableName) {
                    $available = "SELECT * FROM $tableName WHERE name IS NULL AND matric_no IS NULL";
                    $availableResult = $mysqli->query($available);
                    if ($availableResult) {
...
<?php 
    ...
    if ($gender === 'Male') {
        ...
        if (empty($tableNames)) {
            ...
            foreach ($tableNames as $tableName) {
                $available = "SELECT DISTINCT room_no FROM $tableName WHERE name IS NULL AND matric_no IS NULL";
                $availableResult = $mysqli->query($available);
                if ($availableResult) {
                    echo 'HI';
                    while ($row = mysqli_fetch_assoc($availableResult)) {
