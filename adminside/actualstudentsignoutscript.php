...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['matric_no']) && !empty($_POST['matric_no'])) {
        ...
        $hostelname = explode(' ', $_POST['room_no'])[0];

        $name = $_POST['name'];

        // var_dump($matric_no, $hostelname);

        $vacate = vacateStudentFromRoom($matric_no, $name, $hostelname, $conn);
...
function vacateStudentFromRoom($matric_no, $name, $hostelname, $conn) {


    $sql = "UPDATE $hostelname SET matric_no = NULL, name = NULL WHERE matric_no = $matric_no";
            
    // Execute the SQL statement
    if ($conn->query($sql) === TRUE) {
        $message=  $name . " has vacated $hostelname\n";
    } else {
        $message= "Error updating records in $hostelname: " . mysqli_error($conn). "\n";
    }
    return $message;
}
